...
const handleBulkProgramChange = useCallback(async (programId: number | string) => {
  ...
  try {
    ...
    const selectedProgram = bulkFilteredPrograms.find(p => p.id === numericProgramId)
    const isCommonUnits = selectedProgram && (
    ...
    console.log('🔍 Program type detection:', {
      program: selectedProgram?.name,
      is_common_units: isCommonUnits
    })

    if (isCommonUnits) {
      // ✅ FOR ELECTIVES: Get units directly from enrollments
      console.log('📚 Loading elective units from enrollments')
      
      const electivesResponse = await axios.get(
        `/api/electives/available-for-scheduling?semester_id=${bulkFormState.semester_id}&program_id=${numericProgramId}`
      )

      if (electivesResponse.data && Array.isArray(electivesResponse.data)) {
        setAvailableClassesWithUnits(electivesResponse.data)
        toast.success(`Found ${electivesResponse.data.length} elective sections`)
      } else {
        toast.warning("No elective sections found")
        setAvailableClassesWithUnits([])
      }
    ...
    console.log('🔍 Fetching units for regular program')
    ...
    toast.error(error.response?.data?.message || "Failed to load units for the selected program")
    setAvailableClassesWithUnits([])
  } finally {
    setLoadingClassesWithUnits(false)
  }
}, [bulkFormState.semester_id, bulkFilteredPrograms])
