...
const SMSProgramsManagement: React.FC = () => {
  ...
  // 🔥 SMS-SPECIFIC: Medical degree types
  const degreeTypes = [
    { value: 'Certificate', label: 'Certificate in Medical Sciences' },
    { value: 'Diploma', label: 'Diploma in Medical Sciences' },
    { value: 'Bachelor', label: "Bachelor's Degree (Medicine/Nursing)" },
    { value: 'Master', label: "Master's Degree (Clinical/Research)" },
    { value: 'PhD', label: 'Doctoral Degree (MD/PhD)' }
  ]
  ...
  // 🔥 SMS-SPECIFIC: Medical degree badge with Stethoscope icon
  const DegreeBadge: React.FC<{ degreeType: string }> = ({ degreeType }) => {
    const getBadgeColor = (type: string) => {
      switch (type) {
        case 'Certificate': return 'bg-pink-400'
        case 'Diploma': return 'bg-pink-500'
        case 'Bachelor': return 'bg-red-500'
        case 'Master': return 'bg-rose-600'
        case 'PhD': return 'bg-red-700'
  ...
  const handleDeleteProgram = (program: Program) => {
    ...
    if (confirm(`Are you sure you want to delete "${program.name}"? This action cannot be undone.`)) {
      setLoading(true)
      // 🔥 FIXED: Using SMS route
      router.delete(route('schools.sms.programs.destroy', program.id), {
        onSuccess: () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 🔥 FIXED: Using SMS routes
    const url = selectedProgram 
      ? route("schools.sms.programs.update", { program: selectedProgram.id })
      : route("schools.sms.programs.store") 
  ...
  const handleFilter = () => {
    ...
    // 🔥 FIXED: Using SMS route
    router.get(`${route('schools.sms.programs.index')}?${params.toString()}`)
  }
  ...
  return (
    <AuthenticatedLayout>
      ...
      {/* 🔥 SMS BRANDING: Red/Pink/Rose gradient for medical school */}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="flex items-center mb-2">
                    {/* 🔥 SMS ICON: Heart for medical care */}
                    <Heart className="w-8 h-8 text-red-600 mr-3" />
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-slate-800 via-red-700 to-rose-800 bg-clip-text text-transparent">
                      {school.name}
                    </h1>
                  </div>
                  <h2 className="text-2xl font-semibold text-slate-700 mb-2">Medical Programs</h2>
                  <p className="text-slate-600 text-lg">
                    Manage medical, nursing, and health sciences programs
                  </p>
                ...
                <div className="mt-6 sm:mt-0 flex-shrink-0 flex items-center justify-end">
                  {can.create && !isClassTimetableOffice && (
                    <button
                      ...
                      // 🔥 SMS COLORS: Red/Pink/Rose gradient
                      className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-red-500 via-pink-500 to-rose-500 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl hover:from-red-600 hover:via-pink-600 hover:to-rose-600 transform hover:scale-105 hover:-translate-y-0.5 transition-all duration-300 group"
                    >
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search medical programs..."
                    value={searchTerm}
                ...
                <tbody className="divide-y divide-slate-200">
                  {filteredPrograms.map((program, index) => (
                    <React.Fragment key={program.id}>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {can.update && !isClassTimetableOffice && (
                              <button
                                ...
                                className="text-pink-600 hover:text-pink-900 transition-colors p-1 rounded hover:bg-pink-50"
                                title="Edit program"
                              >
                      ...
                      {expandedRows.has(program.id) && (
                        <tr>
                          {/* 🔥 SMS COLORS: Red gradient background */}
                          <td colSpan={6} className="px-6 py-6 bg-red-50/50">
                ...
                <div className="text-center py-12">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm || statusFilter !== 'all' || degreeFilter !== 'all'
                      ...
                      : 'Get started by creating a new medical program'
                    }
                  </p>
                  {can.create && !isClassTimetableOffice && (
                    <button
          ...
          {(isCreateModalOpen || isEditModalOpen) && !isClassTimetableOffice && (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                {/* 🔥 SMS COLORS: Modal header */}
                <div className="bg-gradient-to-r from-red-500 via-pink-500 to-rose-500 p-6 rounded-t-2xl">
                  <h3 className="text-xl font-semibold text-white">
                    {selectedProgram ? 'Edit Medical Program' : 'Create New Medical Program'}
                  </h3>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
                      placeholder="e.g., Medicine & Surgery, Nursing"
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
                      rows={3}
                      placeholder="Brief description of the medical program..."
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
                        placeholder="medicine@strathmore.edu"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                  ...
                  <div className="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="px-6 py-3 bg-gradient-to-r from-red-500 to-pink-600 text-white rounded-lg hover:from-red-600 hover:to-pink-700 transition-all duration-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                    >
          ...
          {isViewModalOpen && selectedProgram && (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                {/* 🔥 SMS COLORS: View modal header */}
                <div className="bg-gradient-to-r from-red-600 via-pink-600 to-rose-600 p-6 rounded-t-2xl">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xl font-semibold text-white">Medical Program Details</h3>
                    <button
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-red-600 font-semibold mt-1">{selectedProgram.code}</p>
...
export default SMSProgramsManagement