...
const DynamicRoles = () => {
    const { roles, stats, permissions, filter, search, perPage } = usePage().props as {
        roles: PaginatedRoles | undefined;
        stats: { total: number; core: number; dynamic: number; } | undefined;
    ...
    const rolesData = roles || { data: [], links: [], total: 0, per_page: 10, current_page: 1 };
    const statsData = stats || { total: 0, core: 0, dynamic: 0 };
    ...
    // Current role being operated on
    ...
    const handleSearch = () => {
        router.get('/admin/roles/dynamic', {
            search: searchQuery,
    ...
    const openEditModal = (role: Role) => {
        ...
        setFormData({
            ...
            permissions: Array.isArray(role.permissions) ? role.permissions.map(p => p.name) : []
        });
    ...
    const handleUpdate = () => {
        ...
        setIsLoading(true);
        router.put(`/admin/roles/${currentRole.id}`, formData, {
            onSuccess: () => {
    ...
    const handleDelete = () => {
        ...
        setIsLoading(true);
        router.delete(`/admin/roles/${currentRole.id}`, {
            onSuccess: () => {
                toast.success('Role deleted successfully!');
                closeAllModals();
            },
            onError: (errors) => {
                const errorMessage = errors.error || 'Failed to delete role.';
    ...
    const handlePermissionUpdate = () => {
        ...
        setIsLoading(true);
        router.put(`/admin/roles/${currentRole.id}/permissions`, {
            ...
            onError: (errors) => {
                const errorMessage = errors.error || 'Failed to update permissions.';
                toast.error(errorMessage);
            },
            onFinish: () => setIsLoading(false)
        });
    ...
    const openBulkCreateModal = () => {
        setBulkRoles([{ name: '', description: '', permissions: [] }]);
        setIsBulkCreateModalOpen(true);
    };

    const addBulkRole = () => {
        setBulkRoles([...bulkRoles, { name: '', description: '', permissions: [] }]);
    };

    const removeBulkRole = (index: number) => {
        setBulkRoles(bulkRoles.filter((_, i) => i !== index));
    };

    const updateBulkRole = (index: number, field: string, value: string | string[]) => {
        const updated = [...bulkRoles];
        updated[index] = { ...updated[index], [field]: value };
        setBulkRoles(updated);
    };

    const handleBulkCreate = () => {
        const validRoles = bulkRoles.filter(r => r.name.trim());
        
        if (validRoles.length === 0) {
            toast.error('Please add at least one role with a name.');
            return;
        }

        setIsLoading(true);
        router.post('/admin/roles/bulk-create', { roles: validRoles }, {
            onSuccess: () => {
                toast.success(`${validRoles.length} roles created successfully!`);
                closeAllModals();
            },
            onError: (errors) => {
                setErrors(errors);
    ...
    const handleSelectAll = () => {
        if (selectedRoles.length === rolesData.data.length) {
            setSelectedRoles([]);
        } else {
            setSelectedRoles(rolesData.data.map(r => r.id));
        }
    };
    ...
    const getPermissionBadgeColor = (permission: Permission) => {
        ...
        return colors[permission.category] || colors['general'];
    };

    return (
        <AuthenticatedLayout>
            <Head title="Dynamic Roles Management" />
            <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8">
                ...
                <div className="bg-white rounded-2xl shadow-xl border p-8 mb-6">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p className="text-slate-600 text-lg">Manage system roles and create dynamic roles with permissions</p>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    ...
                    >
                        <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wider">Total Roles</h3>
                        <p className="text-3xl font-bold text-slate-800">{statsData.total}</p>
                    </div>
                    ...
                    >
                        <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wider">Core Roles</h3>
                        <p className="text-3xl font-bold text-blue-600">{statsData.core}</p>
                    </div>
                    ...
                    >
                        <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wider">Dynamic Roles</h3>
                        <p className="text-3xl font-bold text-emerald-600">{statsData.dynamic}</p>
                    </div>
                </div>
                            ...
                            <thead className="bg-slate-50">
                                <tr>
                                    <th className="px-6 py-4 text-left">
                                        <input
                                            type="checkbox"
                                            checked={selectedRoles.length === rolesData.data.length && rolesData.data.length > 0}
                            ...
                            <tbody className="divide-y divide-slate-200">
                                {rolesData.data.length ? (
                                    rolesData.data.map((role, index) => (
                                        <tr key={role.id} className={`hover:bg-slate-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-slate-25'}`}>
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full border ${getRoleBadgeColor(role)}`}>
                                                    {role.is_core ? 'Core' : 'Dynamic'}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center">
                                                    <span className="text-sm font-medium text-slate-900">{role.users_count}</span>
                                                    <span className="ml-2 text-xs text-slate-500">users</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center">
                                                    ...
                                                    {Array.isArray(role.permissions) && role.permissions.length > 0 && (
                                                        <div className="ml-3 flex flex-wrap gap-1">
                                                            {role.permissions.slice(0, 2).map(permission => (
                                                                <span key={permission.id} className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getPermissionBadgeColor(permission)}`}>
                                                                    {permission.name}
                                                                </span>
                                                            ))}
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex space-x-2">
                                                    ...
                                                    <button
                                                        ...
                                                        className="inline-flex items-center px-3 py-2 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white text-sm font-medium rounded-lg hover:from-indigo-600 hover:to-indigo-700 transform hover:scale-105 transition-all duration-200"
                                                    >
                                                        <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <tr>
                                        <td colSpan={7} className="px-6 py-12 text-center">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <h3 className="text-lg font-medium text-slate-900 mb-1">No roles found</h3>
                                                <p className="text-slate-500">Try adjusting your search or create a new role</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                    ...
                    {rolesData.data.length > 0 && (
                        <div className="px-6 py-4 bg-slate-50 border-t flex items-center justify-between">
                            <p className="text-sm text-slate-600">
                                Showing {rolesData.data.length} of {rolesData.total} roles
                            </p>
                            <div className="flex space-x-2">
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full">
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-2xl font-bold text-slate-800">Create New Role</h3>
                                    <p className="text-slate-600 mt-1">Define a new system role with permissions</p>
                                </div>
                                <button
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="max-h-48 overflow-y-auto border border-slate-200 rounded-xl p-4 bg-slate-50">
                                        <div className="grid grid-cols-2 gap-2">
                                            {permissionsData.map(permission => (
                                                <label key={permission.id} className="flex items-center space-x-2 p-2 hover:bg-white rounded-lg cursor-pointer">
                                                    <input
                                                        type="checkbox"
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="text-sm font-medium text-slate-700">{permission.name}</div>
                                                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getPermissionBadgeColor(permission)}`}>
                                                            {permission.category}
                                                        </span>
                                                    </div>
                                                </label>
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full">
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-2xl font-bold text-slate-800">Edit Role: {currentRole.name}</h3>
                                    <p className="text-slate-600 mt-1">Modify role details and permissions</p>
                                </div>
                                <button
                                    onClick={closeAllModals}
                                    className="p-2 hover:bg-slate-100 rounded-full transition-colors"
            ...
            {/* View Role Modal */}
            {isViewModalOpen && currentRole && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full">
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-2xl font-bold text-slate-800">Role Details: {currentRole.name}</h3>
                                    <div className="flex items-center mt-2 space-x-3">
                                        <span className={`inline-flex px-3 py-1 text-sm font-medium rounded-full border ${getRoleBadgeColor(currentRole)}`}>
                                            {currentRole.is_core ? 'Core Role' : 'Dynamic Role'}
                                        </span>
                                        <span className="text-slate-500">
                                            {currentRole.users_count || 0} users • {Array.isArray(currentRole.permissions) ? currentRole.permissions.length : (currentRole.permissions_count || 0)} permissions
                                        </span>
                                    </div>
                                </div>
                                <button
                            ...
                            <div className="space-y-6">
                                <div className="bg-slate-50 rounded-lg p-4">
                                    <h4 className="text-lg font-semibold text-slate-800 mb-3">Role Information</h4>
                                    <div className="space-y-3">
                                        <div>
                                            <label className="block text-sm font-medium text-slate-600">Name</label>
                                            <p className="text-slate-900 font-medium">{currentRole.name}</p>
                                        </div>
                                        {currentRole.description && (
                                            <div>
                                                <label className="block text-sm font-medium text-slate-600">Description</label>
                                                <p className="text-slate-900">{currentRole.description}</p>
                                            </div>
                                        )}
                                        <div>
                                            <label className="block text-sm font-medium text-slate-600">Created</label>
                                            <p className="text-slate-900">{new Date(currentRole.created_at).toLocaleDateString()}</p>
                                        </div>
                                    </div>
                                </div>

                                {Array.isArray(currentRole.permissions) && currentRole.permissions.length > 0 && (
                                    <div className="bg-slate-50 rounded-lg p-4">
                                        <h4 className="text-lg font-semibold text-slate-800 mb-3">Assigned Permissions</h4>
                                        <div className="grid grid-cols-2 gap-2">
                                            {currentRole.permissions.map(permission => (
                                                <div key={permission.id} className="flex items-center space-x-2 p-2 bg-white rounded-lg">
                                                    <div className="flex-1">
                                                        <div className="text-sm font-medium text-slate-700">{permission.name}</div>
                                                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getPermissionBadgeColor(permission)}`}>
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full">
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-2xl font-bold text-slate-800">Manage Permissions: {currentRole.name}</h3>
                                    <p className="text-slate-600 mt-1">Select permissions for this role</p>
                                </div>
                                <button
                                    onClick={closeAllModals}
                                    className="p-2 hover:bg-slate-100 rounded-full transition-colors"
                            ...
                            <div className="max-h-96 overflow-y-auto border border-slate-200 rounded-xl p-4 bg-slate-50">
                                <div className="grid grid-cols-2 gap-2">
                                    {permissionsData.map(permission => (
                                        <label key={permission.id} className="flex items-center space-x-3 p-3 hover:bg-white rounded-lg cursor-pointer border border-transparent hover:border-slate-200">
                                            <input
                                                type="checkbox"
                                                checked={formData.permissions.includes(permission.name)}
                                                onChange={() => handlePermissionToggle(permission.name)}
                            ...
                            <div className="flex justify-end space-x-4 pt-6 border-t">
                                ...
                                <button
                                    ...
                                    disabled={isLoading}
                                    className="px-6 py-3 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white font-semibold rounded-xl hover:from-indigo-600 hover:to-indigo-700 transform hover:scale-105 transition-all duration-200 disabled:opacity-50 disabled:transform-none"
                                >
                                    {isLoading ? (
                                        <div className="flex items-center">
                                            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2"></div>
                                ...
                                <p className="text-red-800">
                                    ...
                                    {currentRole.users_count > 0 && (
                                        <span className="block mt-2 text-sm">
                                            This role is currently assigned to {currentRole.users_count} user(s).
                                        </span>
                                    )}
                                </p>
                            </div>
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-2xl font-bold text-slate-800">Bulk Create Roles</h3>
                                    <p className="text-slate-600 mt-1">Create multiple roles at once</p>
                                </div>
                                <button
                                    onClick={closeAllModals}
                                    className="p-2 hover:bg-slate-100 rounded-full transition-colors"
                            ...
                            <div className="space-y-4 max-h-96 overflow-y-auto">
                                {bulkRoles.map((role, index) => (
                                    <div key={index} className="p-4 bg-slate-50 rounded-lg space-y-3">
                                        ...
                                        {/* Permissions for bulk role */}
                                        <div>
                                            <label className="block text-sm font-medium text-slate-600 mb-2">Permissions</label>
                                            <div className="max-h-32 overflow-y-auto border border-slate-200 rounded-lg p-2 bg-white">
                                                <div className="grid grid-cols-2 gap-1">
                                                    {permissionsData.slice(0, 6).map(permission => (
                                                        <label key={permission.id} className="flex items-center space-x-2 p-1 hover:bg-slate-50 rounded cursor-pointer">
                                                            <input
                                                                type="checkbox"
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    updateBulkRole(index, 'permissions', newPermissions);
                                                                }}
                                                                className="rounded focus:ring-blue-500 h-3 w-3 text-blue-600 border-gray-300"
                                                            />
                                                            <div className="text-xs text-slate-700">{permission.name}</div>
                                                        </label>
                                                    ))}
                                                </div>
                                                {permissionsData.length > 6 && (
                                                    <div className="text-xs text-slate-500 mt-2 text-center">
                                                        And {permissionsData.length - 6} more permissions available...
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                ...
                                <div className="flex space-x-4">
                                    ...
                                    >
                                        {isLoading ? (
                                            ...
                                            `Create ${bulkRoles.filter(r => r.name.trim()).length} Roles`
                                        )}
                                    </button>
                                </div>
                            </div>
